...
// функция `useState(default)` это хук, который позволяет создать одно поле состояния и метод для его изменения(аналог `.useState()` для одного компонента).
// Метод принимает аргумент - начальное состояние.
// В качестве значения можно указать объект.
